...
async function list(filter) {
  const response = await axios.get(
    `/category?page=${filter.page}&limit=${filter.limit}&category=${filter.category ? filter.category : ''}`,
  );
  return response.data;
}

async function filterCategory(request, filter) {
  const response = await axios.get(`/category?page=${filter.page}&limit=${filter.limit}${request}`);
  return response.data;
}
