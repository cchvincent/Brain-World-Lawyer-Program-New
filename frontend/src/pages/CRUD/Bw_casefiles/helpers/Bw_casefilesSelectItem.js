...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_casefiles/autocomplete`, { params });
  return response.data;
}

const Bw_casefilesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_casefiles.hasPermissionToCreate
});

export default connect(select)(
  Bw_casefilesSelectItem,
);
