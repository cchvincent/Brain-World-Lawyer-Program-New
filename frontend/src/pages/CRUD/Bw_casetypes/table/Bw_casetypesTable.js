...
import * as dataFormat from 'pages/CRUD/Bw_casetypes/table/Bw_casetypesDataFormatters';

import actions from 'actions/bw_casetypes/bw_casetypesListActions';
...
const Bw_casetypesTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Case Types', title: 'casetypes'},{label: 'Description (EN)', title: 'desc_en'},{label: 'Description (CH)', title: 'desc_ch'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
  ...
  const loading = useSelector((store) => store.bw_casetypes.list.loading);
  const rows = useSelector((store) => store.bw_casetypes.list.rows);
  const count = useSelector((store) => store.bw_casetypes.list.count);
  const modalOpen = useSelector((store) => store.bw_casetypes.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_casetypes.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_casetypes/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_casetypes/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_casetypes</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_casetypes'
                    : 'http://localhost:8080/api-docs/#/Bw_casetypes'
            }
        >
            API documentation for bw_casetypes
        </a>
    </div>
    <br/>

<Link to="/admin/bw_casetypes/new">
  <button
...
export default Bw_casetypesTable;
