...
import * as dataFormat from 'pages/CRUD/Bw_refnofilesseq/table/Bw_refnofilesseqDataFormatters';

import actions from 'actions/bw_refnofilesseq/bw_refnofilesseqListActions';
...
const Bw_refnofilesseqTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Ref No', title: 'refno'},{label: 'Case File Name', title: 'casefilename'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
    {label: 'Client ID', title: 'clientid', number: 'true'},{label: 'File Seq', title: 'fileseq', number: 'true'},
  ...
  const loading = useSelector((store) => store.bw_refnofilesseq.list.loading);
  const rows = useSelector((store) => store.bw_refnofilesseq.list.rows);
  const count = useSelector((store) => store.bw_refnofilesseq.list.count);
  const modalOpen = useSelector((store) => store.bw_refnofilesseq.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_refnofilesseq.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_refnofilesseq/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_refnofilesseq/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_refnofilesseq</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_refnofilesseq'
                    : 'http://localhost:8080/api-docs/#/Bw_refnofilesseq'
            }
        >
            API documentation for bw_refnofilesseq
        </a>
    </div>
    <br/>

<Link to="/admin/bw_refnofilesseq/new">
  <button
...
export default Bw_refnofilesseqTable;
