...
import * as dataFormat from 'pages/CRUD/Bw_supervisors/table/Bw_supervisorsDataFormatters';

import actions from 'actions/bw_supervisors/bw_supervisorsListActions';
...
const Bw_supervisorsTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Name (EN)', title: 'name_en'},{label: 'Name (CH)', title: 'name_ch'},{label: 'Initial', title: 'initial'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
  ...
  const loading = useSelector((store) => store.bw_supervisors.list.loading);
  const rows = useSelector((store) => store.bw_supervisors.list.rows);
  const count = useSelector((store) => store.bw_supervisors.list.count);
  const modalOpen = useSelector((store) => store.bw_supervisors.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_supervisors.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_supervisors/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_supervisors/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_supervisors</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_supervisors'
                    : 'http://localhost:8080/api-docs/#/Bw_supervisors'
            }
        >
            API documentation for bw_supervisors
        </a>
    </div>
    <br/>

<Link to="/admin/bw_supervisors/new">
  <button
...
export default Bw_supervisorsTable;
