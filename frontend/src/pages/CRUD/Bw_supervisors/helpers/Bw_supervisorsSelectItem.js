...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_supervisors/autocomplete`, { params });
  return response.data;
}

const Bw_supervisorsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_supervisors.hasPermissionToCreate
});
