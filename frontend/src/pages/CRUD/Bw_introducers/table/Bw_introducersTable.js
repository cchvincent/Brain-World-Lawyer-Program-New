...
import * as dataFormat from 'pages/CRUD/Bw_introducers/table/Bw_introducersDataFormatters';

import actions from 'actions/bw_introducers/bw_introducersListActions';
...
const Bw_introducersTable = () => {
  ...
  const loading = useSelector((store) => store.bw_introducers.list.loading);
  const rows = useSelector((store) => store.bw_introducers.list.rows);
  const count = useSelector((store) => store.bw_introducers.list.count);
  const modalOpen = useSelector((store) => store.bw_introducers.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_introducers.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_introducers/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_introducers/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_introducers</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_introducers'
                    : 'http://localhost:8080/api-docs/#/Bw_introducers'
            }
        >
            API documentation for bw_introducers
        </a>
    </div>
    <br/>

<Link to="/admin/bw_introducers/new">
  <button
...
export default Bw_introducersTable;
