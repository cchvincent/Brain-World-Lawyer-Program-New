...
import * as dataFormat from 'pages/CRUD/Introducers/table/IntroducersDataFormatters';

import actions from 'actions/introducers/introducersListActions';
...
const IntroducersTable = () => {
  ...
  const loading = useSelector((store) => store.introducers.list.loading);
  const rows = useSelector((store) => store.introducers.list.rows);
  const count = useSelector((store) => store.introducers.list.count);
  const modalOpen = useSelector((store) => store.introducers.list.modalOpen);
  const idToDelete = useSelector((store) => store.introducers.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/introducers/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/introducers/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Introducers</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Introducers'
                    : 'http://localhost:8080/api-docs/#/Introducers'
            }
        >
            API documentation for introducers
        </a>
    </div>
    <br/>

<Link to="/admin/introducers/new">
  <button
...
export default IntroducersTable;
