...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/introducers/autocomplete`, { params });
  return response.data;
}

const IntroducersSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.introducers.hasPermissionToCreate
});

export default connect(select)(
  IntroducersSelectItem,
);
