...
import * as dataFormat from 'pages/CRUD/Bw_caseheaderreferences/table/Bw_caseheaderreferencesDataFormatters';

import actions from 'actions/bw_caseheaderreferences/bw_caseheaderreferencesListActions';
...
const Bw_caseheaderreferencesTable = () => {
  ...
  const loading = useSelector((store) => store.bw_caseheaderreferences.list.loading);
  const rows = useSelector((store) => store.bw_caseheaderreferences.list.rows);
  const count = useSelector((store) => store.bw_caseheaderreferences.list.count);
  const modalOpen = useSelector((store) => store.bw_caseheaderreferences.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_caseheaderreferences.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_caseheaderreferences/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_caseheaderreferences/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_caseheaderreferences</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_caseheaderreferences'
                    : 'http://localhost:8080/api-docs/#/Bw_caseheaderreferences'
            }
        >
            API documentation for bw_caseheaderreferences
        </a>
    </div>
    <br/>

<Link to="/admin/bw_caseheaderreferences/new">
  <button
...
export default Bw_caseheaderreferencesTable;
