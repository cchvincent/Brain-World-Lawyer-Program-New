...
import * as dataFormat from 'pages/CRUD/Files/table/FilesDataFormatters';

import actions from 'actions/files/filesListActions';
...
const FilesTable = () => {
  ...
  const loading = useSelector((store) => store.files.list.loading);
  const rows = useSelector((store) => store.files.list.rows);
  const count = useSelector((store) => store.files.list.count);
  const modalOpen = useSelector((store) => store.files.list.modalOpen);
  const idToDelete = useSelector((store) => store.files.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/files/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/files/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Files</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Files'
                    : 'http://localhost:8080/api-docs/#/Files'
            }
        >
            API documentation for files
        </a>
    </div>
    <br/>
