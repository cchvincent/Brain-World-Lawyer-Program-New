...
import * as dataFormat from 'pages/CRUD/Casetypes/table/CasetypesDataFormatters';

import actions from 'actions/casetypes/casetypesListActions';
...
const CasetypesTable = () => {
  ...
  const loading = useSelector((store) => store.casetypes.list.loading);
  const rows = useSelector((store) => store.casetypes.list.rows);
  const count = useSelector((store) => store.casetypes.list.count);
  const modalOpen = useSelector((store) => store.casetypes.list.modalOpen);
  const idToDelete = useSelector((store) => store.casetypes.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/casetypes/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/casetypes/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Casetypes</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Casetypes'
                    : 'http://localhost:8080/api-docs/#/Casetypes'
            }
        >
            API documentation for casetypes
        </a>
    </div>
    <br/>

<Link to="/admin/casetypes/new">
  <button
