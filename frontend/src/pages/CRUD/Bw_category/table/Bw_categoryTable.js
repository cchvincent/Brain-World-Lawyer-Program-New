...
import * as dataFormat from 'pages/CRUD/Bw_category/table/Bw_categoryDataFormatters';

import actions from 'actions/bw_category/bw_categoryListActions';
...
const Bw_categoryTable = () => {
  ...
  const loading = useSelector((store) => store.bw_category.list.loading);
  const rows = useSelector((store) => store.bw_category.list.rows);
  const count = useSelector((store) => store.bw_category.list.count);
  const modalOpen = useSelector((store) => store.bw_category.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_category.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_category/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_category/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_category</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_category'
                    : 'http://localhost:8080/api-docs/#/Bw_category'
            }
        >
            API documentation for bw_category
        </a>
    </div>
    <br/>

<Link to="/admin/bw_category/new">
  <button
...
export default Bw_categoryTable;
