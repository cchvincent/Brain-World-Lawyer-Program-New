...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_category/autocomplete`, { params });
  return response.data;
}

const Bw_categorySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_category.hasPermissionToCreate
});

export default connect(select)(
  Bw_categorySelectItem,
);
