...
import * as dataFormat from 'pages/CRUD/Casestatus/table/CasestatusDataFormatters';

import actions from 'actions/casestatus/casestatusListActions';
...
const CasestatusTable = () => {
  ...
  const loading = useSelector((store) => store.casestatus.list.loading);
  const rows = useSelector((store) => store.casestatus.list.rows);
  const count = useSelector((store) => store.casestatus.list.count);
  const modalOpen = useSelector((store) => store.casestatus.list.modalOpen);
  const idToDelete = useSelector((store) => store.casestatus.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/casestatus/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/casestatus/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Casestatus</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Casestatus'
                    : 'http://localhost:8080/api-docs/#/Casestatus'
            }
        >
            API documentation for casestatus
        </a>
    </div>
    <br/>

<Link to="/admin/casestatus/new">
  <button
