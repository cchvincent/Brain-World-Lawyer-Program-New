...
import * as dataFormat from 'pages/CRUD/Refnofilesseq/table/RefnofilesseqDataFormatters';

import actions from 'actions/refnofilesseq/refnofilesseqListActions';
...
const RefnofilesseqTable = () => {
  ...
  const loading = useSelector((store) => store.refnofilesseq.list.loading);
  const rows = useSelector((store) => store.refnofilesseq.list.rows);
  const count = useSelector((store) => store.refnofilesseq.list.count);
  const modalOpen = useSelector((store) => store.refnofilesseq.list.modalOpen);
  const idToDelete = useSelector((store) => store.refnofilesseq.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/refnofilesseq/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/refnofilesseq/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Refnofilesseq</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Refnofilesseq'
                    : 'http://localhost:8080/api-docs/#/Refnofilesseq'
            }
        >
            API documentation for refnofilesseq
        </a>
    </div>
    <br/>

<Link to="/admin/refnofilesseq/new">
  <button
...
export default RefnofilesseqTable;
