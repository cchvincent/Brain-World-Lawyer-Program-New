...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/refnofilesseq/autocomplete`, { params });
  return response.data;
}

const RefnofilesseqSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.refnofilesseq.hasPermissionToCreate
});

export default connect(select)(
  RefnofilesseqSelectItem,
);
