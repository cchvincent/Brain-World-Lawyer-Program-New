...
import * as dataFormat from 'pages/CRUD/Progress/table/ProgressDataFormatters';

import actions from 'actions/progress/progressListActions';
...
const ProgressTable = () => {
  ...
  const loading = useSelector((store) => store.progress.list.loading);
  const rows = useSelector((store) => store.progress.list.rows);
  const count = useSelector((store) => store.progress.list.count);
  const modalOpen = useSelector((store) => store.progress.list.modalOpen);
  const idToDelete = useSelector((store) => store.progress.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/progress/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/progress/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Progress</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Progress'
                    : 'http://localhost:8080/api-docs/#/Progress'
            }
        >
            API documentation for progress
        </a>
    </div>
    <br/>

<Link to="/admin/progress/new">
  <button
