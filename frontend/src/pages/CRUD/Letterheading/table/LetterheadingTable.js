...
import * as dataFormat from 'pages/CRUD/Letterheading/table/LetterheadingDataFormatters';

import actions from 'actions/letterheading/letterheadingListActions';
...
const LetterheadingTable = () => {
  ...
  const loading = useSelector((store) => store.letterheading.list.loading);
  const rows = useSelector((store) => store.letterheading.list.rows);
  const count = useSelector((store) => store.letterheading.list.count);
  const modalOpen = useSelector((store) => store.letterheading.list.modalOpen);
  const idToDelete = useSelector((store) => store.letterheading.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/letterheading/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/letterheading/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Letterheading</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Letterheading'
                    : 'http://localhost:8080/api-docs/#/Letterheading'
            }
        >
            API documentation for letterheading
        </a>
    </div>
    <br/>

<Link to="/admin/letterheading/new">
  <button
...
export default LetterheadingTable;
