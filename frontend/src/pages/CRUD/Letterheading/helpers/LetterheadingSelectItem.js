...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/letterheading/autocomplete`, { params });
  return response.data;
}

const LetterheadingSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.letterheading.hasPermissionToCreate
});

export default connect(select)(
  LetterheadingSelectItem,
);
