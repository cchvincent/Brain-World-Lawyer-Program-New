...
// eslint-disable-next-line
import * as bw_clientsDataFormat from 'pages/CRUD/Bw_clients/table/Bw_clientsDataFormatters';

import actions from 'actions/bw_caseheader/bw_caseheaderListActions';
...
const Bw_caseheaderTable = () => {
  ...
  const loading = useSelector((store) => store.bw_caseheader.list.loading);
  const rows = useSelector((store) => store.bw_caseheader.list.rows);
  const count = useSelector((store) => store.bw_caseheader.list.count);
  const modalOpen = useSelector((store) => store.bw_caseheader.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_caseheader.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_caseheader/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_caseheader/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
sort: true,
  ...
  formatter: bw_casetypesDataFormat.listFormatter,
...
sort: true,
  ...
  formatter: bw_officesDataFormat.listFormatter,

text: "Case File Location ID"
},

{ dataField: "clientid",
sort: true,

  formatter: bw_clientsDataFormat.listFormatter,
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_caseheader</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_caseheader'
                    : 'http://localhost:8080/api-docs/#/Bw_caseheader'
            }
        >
            API documentation for bw_caseheader
        </a>
    </div>
    <br/>

<Link to="/admin/bw_caseheader/new">
  <button
...
export default Bw_caseheaderTable;
