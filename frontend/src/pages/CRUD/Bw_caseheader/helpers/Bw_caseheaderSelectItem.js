...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_caseheader/autocomplete`, { params });
  return response.data;
}

const Bw_caseheaderSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_caseheader.hasPermissionToCreate
});

export default connect(select)(
  Bw_caseheaderSelectItem,
);
