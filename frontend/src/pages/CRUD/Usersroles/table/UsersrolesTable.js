...
import * as dataFormat from 'pages/CRUD/Usersroles/table/UsersrolesDataFormatters';

import actions from 'actions/usersroles/usersrolesListActions';
...
const UsersrolesTable = () => {
  ...
  const loading = useSelector((store) => store.usersroles.list.loading);
  const rows = useSelector((store) => store.usersroles.list.rows);
  const count = useSelector((store) => store.usersroles.list.count);
  const modalOpen = useSelector((store) => store.usersroles.list.modalOpen);
  const idToDelete = useSelector((store) => store.usersroles.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/usersroles/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/usersroles/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Usersroles</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Usersroles'
                    : 'http://localhost:8080/api-docs/#/Usersroles'
            }
        >
            API documentation for usersroles
        </a>
    </div>
    <br/>

<Link to="/admin/usersroles/new">
  <button
