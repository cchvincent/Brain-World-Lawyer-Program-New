...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_caseserial/autocomplete`, { params });
  return response.data;
}

const Bw_caseserialSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_caseserial.hasPermissionToCreate
});

export default connect(select)(
  Bw_caseserialSelectItem,
);
