...
import * as dataFormat from 'pages/CRUD/Bw_caseserial/table/Bw_caseserialDataFormatters';

import actions from 'actions/bw_caseserial/bw_caseserialListActions';
...
const Bw_caseserialTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Serial Code', title: 'serialcode'},
  ...
  const loading = useSelector((store) => store.bw_caseserial.list.loading);
  const rows = useSelector((store) => store.bw_caseserial.list.rows);
  const count = useSelector((store) => store.bw_caseserial.list.count);
  const modalOpen = useSelector((store) => store.bw_caseserial.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_caseserial.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_caseserial/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_caseserial/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_caseserial</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_caseserial'
                    : 'http://localhost:8080/api-docs/#/Bw_caseserial'
            }
        >
            API documentation for bw_caseserial
        </a>
    </div>
    <br/>

<Link to="/admin/bw_caseserial/new">
  <button
...
export default Bw_caseserialTable;
