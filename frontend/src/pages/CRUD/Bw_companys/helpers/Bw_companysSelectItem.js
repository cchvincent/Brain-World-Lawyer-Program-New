...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_companys/autocomplete`, { params });
  return response.data;
}

const Bw_companysSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_companys.hasPermissionToCreate
});

export default connect(select)(
  Bw_companysSelectItem,
);
