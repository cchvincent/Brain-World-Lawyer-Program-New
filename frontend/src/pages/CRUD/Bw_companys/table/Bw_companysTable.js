...
import * as dataFormat from 'pages/CRUD/Bw_companys/table/Bw_companysDataFormatters';

import actions from 'actions/bw_companys/bw_companysListActions';
...
const Bw_companysTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Name (EN)', title: 'name_en'},{label: 'Name (CH)', title: 'name_ch'},{label: 'Address (EN)', title: 'address_en'},{label: 'Address (CH)', title: 'address_ch'},{label: 'Slogan (EN)', title: 'slogan_en'},{label: 'Slogan (CH)', title: 'slogan_ch'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
  ...
  const loading = useSelector((store) => store.bw_companys.list.loading);
  const rows = useSelector((store) => store.bw_companys.list.rows);
  const count = useSelector((store) => store.bw_companys.list.count);
  const modalOpen = useSelector((store) => store.bw_companys.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_companys.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_companys/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_companys/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_companys</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_companys'
                    : 'http://localhost:8080/api-docs/#/Bw_companys'
            }
        >
            API documentation for bw_companys
        </a>
    </div>
    <br/>

<Link to="/admin/bw_companys/new">
  <button
...
export default Bw_companysTable;
