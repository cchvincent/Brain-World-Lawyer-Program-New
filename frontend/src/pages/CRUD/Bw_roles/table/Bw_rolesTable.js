...
import * as dataFormat from 'pages/CRUD/Bw_roles/table/Bw_rolesDataFormatters';

import actions from 'actions/bw_roles/bw_rolesListActions';
...
const Bw_rolesTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Name (EN)', title: 'name_en'},{label: 'Name (CH)', title: 'name_ch'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
  ...
  const loading = useSelector((store) => store.bw_roles.list.loading);
  const rows = useSelector((store) => store.bw_roles.list.rows);
  const count = useSelector((store) => store.bw_roles.list.count);
  const modalOpen = useSelector((store) => store.bw_roles.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_roles.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_roles/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_roles/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_roles</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_roles'
                    : 'http://localhost:8080/api-docs/#/Bw_roles'
            }
        >
            API documentation for bw_roles
        </a>
    </div>
    <br/>

<Link to="/admin/bw_roles/new">
  <button
