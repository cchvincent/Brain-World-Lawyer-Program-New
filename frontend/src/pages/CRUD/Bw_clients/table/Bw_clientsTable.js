...
import * as dataFormat from 'pages/CRUD/Bw_clients/table/Bw_clientsDataFormatters';

import actions from 'actions/bw_clients/bw_clientsListActions';
...
const Bw_clientsTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Name (EN)', title: 'name_en'},{label: 'Name (CH)', title: 'name_ch'},{label: 'Address (EN)', title: 'address_en'},{label: 'Address (CH)', title: 'address_ch'},{label: 'Contact No', title: 'contactno'},{label: 'Contact Person (EN)', title: 'contactperson_en'},{label: 'Contact Person (CH)', title: 'contactperson_ch'},{label: 'Mobile', title: 'mobile'},{label: 'HKIC', title: 'hkic'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update by', title: 'updateby'},
  ...
  const loading = useSelector((store) => store.bw_clients.list.loading);
  const rows = useSelector((store) => store.bw_clients.list.rows);
  const count = useSelector((store) => store.bw_clients.list.count);
  const modalOpen = useSelector((store) => store.bw_clients.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_clients.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_clients/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_clients/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
{ dataField: "contactperson_en",
sort: true,

text: "Contact Person (EN)"
},

{ dataField: "contactperson_ch",
sort: true,

text: "Contact Person (CH)"
},
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_clients</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_clients'
                    : 'http://localhost:8080/api-docs/#/Bw_clients'
            }
        >
            API documentation for bw_clients
        </a>
    </div>
    <br/>

<Link to="/admin/bw_clients/new">
  <button
...
export default Bw_clientsTable;
