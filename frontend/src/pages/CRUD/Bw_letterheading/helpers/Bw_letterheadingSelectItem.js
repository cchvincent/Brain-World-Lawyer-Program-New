...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_letterheading/autocomplete`, { params });
  return response.data;
}

const Bw_letterheadingSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_letterheading.hasPermissionToCreate
});

export default connect(select)(
  Bw_letterheadingSelectItem,
);
