...
import * as dataFormat from 'pages/CRUD/Bw_letterheading/table/Bw_letterheadingDataFormatters';

import actions from 'actions/bw_letterheading/bw_letterheadingListActions';
...
const Bw_letterheadingTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Description (EN)', title: 'desc_en'},{label: 'Description (CH)', title: 'desc_ch'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
  ...
  const loading = useSelector((store) => store.bw_letterheading.list.loading);
  const rows = useSelector((store) => store.bw_letterheading.list.rows);
  const count = useSelector((store) => store.bw_letterheading.list.count);
  const modalOpen = useSelector((store) => store.bw_letterheading.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_letterheading.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_letterheading/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_letterheading/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_letterheading</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_letterheading'
                    : 'http://localhost:8080/api-docs/#/Bw_letterheading'
            }
        >
            API documentation for bw_letterheading
        </a>
    </div>
    <br/>

<Link to="/admin/bw_letterheading/new">
  <button
...
export default Bw_letterheadingTable;
