...
import * as dataFormat from 'pages/CRUD/Handlers/table/HandlersDataFormatters';

import actions from 'actions/handlers/handlersListActions';
...
const HandlersTable = () => {
  ...
  const loading = useSelector((store) => store.handlers.list.loading);
  const rows = useSelector((store) => store.handlers.list.rows);
  const count = useSelector((store) => store.handlers.list.count);
  const modalOpen = useSelector((store) => store.handlers.list.modalOpen);
  const idToDelete = useSelector((store) => store.handlers.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/handlers/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/handlers/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Handlers</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Handlers'
                    : 'http://localhost:8080/api-docs/#/Handlers'
            }
        >
