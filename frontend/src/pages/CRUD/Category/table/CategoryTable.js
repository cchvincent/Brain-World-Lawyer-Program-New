...
import * as dataFormat from 'pages/CRUD/Category/table/CategoryDataFormatters';

import actions from 'actions/category/categoryListActions';
...
const CategoryTable = () => {
  ...
  const loading = useSelector((store) => store.category.list.loading);
  const rows = useSelector((store) => store.category.list.rows);
  const count = useSelector((store) => store.category.list.count);
  const modalOpen = useSelector((store) => store.category.list.modalOpen);
  const idToDelete = useSelector((store) => store.category.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/category/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/category/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Category</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Category'
                    : 'http://localhost:8080/api-docs/#/Category'
            }
        >
            API documentation for category
        </a>
    </div>
    <br/>
