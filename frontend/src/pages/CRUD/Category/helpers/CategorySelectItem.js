...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/category/autocomplete`, { params });
  return response.data;
}

const CategorySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.category.hasPermissionToCreate,
});

export default connect(select)(CategorySelectItem);
