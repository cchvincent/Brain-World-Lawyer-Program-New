...
import * as dataFormat from 'pages/CRUD/Bw_handlers/table/Bw_handlersDataFormatters';

import actions from 'actions/bw_handlers/bw_handlersListActions';
...
const Bw_handlersTable = () => {
  ...
  const loading = useSelector((store) => store.bw_handlers.list.loading);
  const rows = useSelector((store) => store.bw_handlers.list.rows);
  const count = useSelector((store) => store.bw_handlers.list.count);
  const modalOpen = useSelector((store) => store.bw_handlers.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_handlers.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_handlers/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_handlers/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_handlers</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_handlers'
                    : 'http://localhost:8080/api-docs/#/Bw_handlers'
            }
        >
            API documentation for bw_handlers
        </a>
    </div>
    <br/>

<Link to="/admin/bw_handlers/new">
  <button
...
export default Bw_handlersTable;
