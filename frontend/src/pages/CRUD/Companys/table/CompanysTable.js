...
import * as dataFormat from 'pages/CRUD/Companys/table/CompanysDataFormatters';

import actions from 'actions/companys/companysListActions';
...
const CompanysTable = () => {
  ...
  const loading = useSelector((store) => store.companys.list.loading);
  const rows = useSelector((store) => store.companys.list.rows);
  const count = useSelector((store) => store.companys.list.count);
  const modalOpen = useSelector((store) => store.companys.list.modalOpen);
  const idToDelete = useSelector((store) => store.companys.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/companys/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/companys/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Companys</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Companys'
                    : 'http://localhost:8080/api-docs/#/Companys'
            }
        >
            API documentation for companys
        </a>
    </div>
    <br/>

<Link to="/admin/companys/new">
  <button
