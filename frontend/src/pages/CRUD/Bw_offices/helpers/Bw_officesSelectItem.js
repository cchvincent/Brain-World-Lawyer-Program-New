...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_offices/autocomplete`, { params });
  return response.data;
}

const Bw_officesSelectItem = (props) => {
  ...
  const mapper = {
    intoSelect(originalValue) {
      ...
      let label = originalValue.label ? originalValue.label : originalValue.officetypes;
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_offices.hasPermissionToCreate
});
