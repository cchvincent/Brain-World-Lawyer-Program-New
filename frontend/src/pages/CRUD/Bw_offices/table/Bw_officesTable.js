...
import * as dataFormat from 'pages/CRUD/Bw_offices/table/Bw_officesDataFormatters';

import actions from 'actions/bw_offices/bw_officesListActions';
...
const Bw_officesTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Office Types', title: 'officetypes'},{label: 'Description (EN)', title: 'desc_en'},{label: 'Description (CH)', title: 'desc_ch'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
  ...
  const loading = useSelector((store) => store.bw_offices.list.loading);
  const rows = useSelector((store) => store.bw_offices.list.rows);
  const count = useSelector((store) => store.bw_offices.list.count);
  const modalOpen = useSelector((store) => store.bw_offices.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_offices.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_offices/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_offices/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_offices</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_offices'
                    : 'http://localhost:8080/api-docs/#/Bw_offices'
            }
        >
            API documentation for bw_offices
        </a>
    </div>
    <br/>

<Link to="/admin/bw_offices/new">
  <button
...
export default Bw_officesTable;
