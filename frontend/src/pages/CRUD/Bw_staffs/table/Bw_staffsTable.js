...
import * as dataFormat from 'pages/CRUD/Bw_staffs/table/Bw_staffsDataFormatters';

import actions from 'actions/bw_staffs/bw_staffsListActions';
...
const Bw_staffsTable = () => {
  ...
  const loading = useSelector((store) => store.bw_staffs.list.loading);
  const rows = useSelector((store) => store.bw_staffs.list.rows);
  const count = useSelector((store) => store.bw_staffs.list.count);
  const modalOpen = useSelector((store) => store.bw_staffs.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_staffs.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_staffs/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_staffs/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_staffs</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_staffs'
                    : 'http://localhost:8080/api-docs/#/Bw_staffs'
            }
        >
            API documentation for bw_staffs
        </a>
    </div>
    <br/>

<Link to="/admin/bw_staffs/new">
  <button
