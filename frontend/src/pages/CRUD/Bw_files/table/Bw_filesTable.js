...
import * as dataFormat from 'pages/CRUD/Bw_files/table/Bw_filesDataFormatters';

import actions from 'actions/bw_files/bw_filesListActions';
...
const Bw_filesTable = () => {
  ...
  const loading = useSelector((store) => store.bw_files.list.loading);
  const rows = useSelector((store) => store.bw_files.list.rows);
  const count = useSelector((store) => store.bw_files.list.count);
  const modalOpen = useSelector((store) => store.bw_files.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_files.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_files/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_files/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_files</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_files'
                    : 'http://localhost:8080/api-docs/#/Bw_files'
            }
        >
            API documentation for bw_files
        </a>
    </div>
    <br/>

<Link to="/admin/bw_files/new">
  <button
