...
import * as dataFormat from 'pages/CRUD/Bw_progress/table/Bw_progressDataFormatters';

import actions from 'actions/bw_progress/bw_progressListActions';
...
const Bw_progressTable = () => {
  const [filters, setFilters] = React.useState([
    ...
    {label: 'Progress Types', title: 'progresstypes', number: 'true'},
  ...
  const loading = useSelector((store) => store.bw_progress.list.loading);
  const rows = useSelector((store) => store.bw_progress.list.rows);
  const count = useSelector((store) => store.bw_progress.list.count);
  const modalOpen = useSelector((store) => store.bw_progress.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_progress.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_progress/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_progress/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_progress</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_progress'
                    : 'http://localhost:8080/api-docs/#/Bw_progress'
            }
        >
            API documentation for bw_progress
        </a>
    </div>
    <br/>

<Link to="/admin/bw_progress/new">
  <button
...
export default Bw_progressTable;
