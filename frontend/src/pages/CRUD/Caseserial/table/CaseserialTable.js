...
import * as dataFormat from 'pages/CRUD/Caseserial/table/CaseserialDataFormatters';

import actions from 'actions/caseserial/caseserialListActions';
...
const CaseserialTable = () => {
  ...
  const loading = useSelector((store) => store.caseserial.list.loading);
  const rows = useSelector((store) => store.caseserial.list.rows);
  const count = useSelector((store) => store.caseserial.list.count);
  const modalOpen = useSelector((store) => store.caseserial.list.modalOpen);
  const idToDelete = useSelector((store) => store.caseserial.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/caseserial/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/caseserial/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Caseserial</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Caseserial'
                    : 'http://localhost:8080/api-docs/#/Caseserial'
            }
        >
            API documentation for caseserial
        </a>
    </div>
    <br/>

<Link to="/admin/caseserial/new">
  <button
...
export default CaseserialTable;
