...
import * as dataFormat from 'pages/CRUD/Filedocs/table/FiledocsDataFormatters';

import actions from 'actions/filedocs/filedocsListActions';
...
const FiledocsTable = () => {
  ...
  const loading = useSelector((store) => store.filedocs.list.loading);
  const rows = useSelector((store) => store.filedocs.list.rows);
  const count = useSelector((store) => store.filedocs.list.count);
  const modalOpen = useSelector((store) => store.filedocs.list.modalOpen);
  const idToDelete = useSelector((store) => store.filedocs.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/filedocs/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/filedocs/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Filedocs</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Filedocs'
                    : 'http://localhost:8080/api-docs/#/Filedocs'
            }
        >
            API documentation for filedocs
        </a>
    </div>
    <br/>

<Link to="/admin/filedocs/new">
  <button
