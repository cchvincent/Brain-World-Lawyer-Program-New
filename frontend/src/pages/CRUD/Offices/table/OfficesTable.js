...
import * as dataFormat from 'pages/CRUD/Offices/table/OfficesDataFormatters';

import actions from 'actions/offices/officesListActions';
...
const OfficesTable = () => {
  ...
  const loading = useSelector((store) => store.offices.list.loading);
  const rows = useSelector((store) => store.offices.list.rows);
  const count = useSelector((store) => store.offices.list.count);
  const modalOpen = useSelector((store) => store.offices.list.modalOpen);
  const idToDelete = useSelector((store) => store.offices.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/offices/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/offices/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Offices</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Offices'
                    : 'http://localhost:8080/api-docs/#/Offices'
            }
        >
            API documentation for offices
        </a>
    </div>
    <br/>
