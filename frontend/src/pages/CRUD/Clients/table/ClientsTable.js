...
import * as dataFormat from 'pages/CRUD/Clients/table/ClientsDataFormatters';

import actions from 'actions/clients/clientsListActions';
...
const ClientsTable = () => {
  ...
  const loading = useSelector((store) => store.clients.list.loading);
  const rows = useSelector((store) => store.clients.list.rows);
  const count = useSelector((store) => store.clients.list.count);
  const modalOpen = useSelector((store) => store.clients.list.modalOpen);
  const idToDelete = useSelector((store) => store.clients.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/clients/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/clients/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Clients</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Clients'
                    : 'http://localhost:8080/api-docs/#/Clients'
            }
        >
