...
import * as dataFormat from 'pages/CRUD/Bw_filedocs/table/Bw_filedocsDataFormatters';

import actions from 'actions/bw_filedocs/bw_filedocsListActions';
...
const Bw_filedocsTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Ref No', title: 'refno'},{label: 'Doc Box Name', title: 'docboxname'},{label: 'Folder Path', title: 'folderpath'},{label: 'File Name', title: 'filename'},{label: 'Status', title: 'status'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
    {label: 'Parent ID', title: 'parentid', number: 'true'},
  ...
  const loading = useSelector((store) => store.bw_filedocs.list.loading);
  const rows = useSelector((store) => store.bw_filedocs.list.rows);
  const count = useSelector((store) => store.bw_filedocs.list.count);
  const modalOpen = useSelector((store) => store.bw_filedocs.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_filedocs.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_filedocs/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_filedocs/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_filedocs</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_filedocs'
                    : 'http://localhost:8080/api-docs/#/Bw_filedocs'
            }
        >
            API documentation for bw_filedocs
        </a>
    </div>
    <br/>

<Link to="/admin/bw_filedocs/new">
  <button
...
export default Bw_filedocsTable;
