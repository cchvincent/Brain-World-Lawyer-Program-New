...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bw_filedocs/autocomplete`, { params });
  return response.data;
}

const Bw_filedocsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bw_filedocs.hasPermissionToCreate
});

export default connect(select)(
  Bw_filedocsSelectItem,
);
