...
import * as dataFormat from 'pages/CRUD/Supervisors/table/SupervisorsDataFormatters';

import actions from 'actions/supervisors/supervisorsListActions';
...
const SupervisorsTable = () => {
  ...
  const loading = useSelector((store) => store.supervisors.list.loading);
  const rows = useSelector((store) => store.supervisors.list.rows);
  const count = useSelector((store) => store.supervisors.list.count);
  const modalOpen = useSelector((store) => store.supervisors.list.modalOpen);
  const idToDelete = useSelector((store) => store.supervisors.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/supervisors/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/supervisors/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Supervisors</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Supervisors'
                    : 'http://localhost:8080/api-docs/#/Supervisors'
            }
        >
            API documentation for supervisors
        </a>
    </div>
    <br/>
