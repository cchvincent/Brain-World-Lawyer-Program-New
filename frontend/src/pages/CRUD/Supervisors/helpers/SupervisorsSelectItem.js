...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/supervisors/autocomplete`, { params });
  return response.data;
}

const SupervisorsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.supervisors.hasPermissionToCreate
});

export default connect(select)(
  SupervisorsSelectItem,
);
