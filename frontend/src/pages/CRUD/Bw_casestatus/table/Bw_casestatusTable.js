...
import * as dataFormat from 'pages/CRUD/Bw_casestatus/table/Bw_casestatusDataFormatters';

import actions from 'actions/bw_casestatus/bw_casestatusListActions';
...
const Bw_casestatusTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Description (EN)', title: 'desc_en'},{label: 'Description (CH)', title: 'desc_ch'},{label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},{label: 'Status', title: 'status'},
  ...
  const loading = useSelector((store) => store.bw_casestatus.list.loading);
  const rows = useSelector((store) => store.bw_casestatus.list.rows);
  const count = useSelector((store) => store.bw_casestatus.list.count);
  const modalOpen = useSelector((store) => store.bw_casestatus.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_casestatus.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_casestatus/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_casestatus/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_casestatus</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_casestatus'
                    : 'http://localhost:8080/api-docs/#/Bw_casestatus'
            }
        >
            API documentation for bw_casestatus
        </a>
    </div>
    <br/>

<Link to="/admin/bw_casestatus/new">
  <button
...
export default Bw_casestatusTable;
