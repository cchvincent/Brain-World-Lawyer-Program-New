...
import * as dataFormat from 'pages/CRUD/Staffs/table/StaffsDataFormatters';

import actions from 'actions/staffs/staffsListActions';
...
const StaffsTable = () => {
  ...
  const loading = useSelector((store) => store.staffs.list.loading);
  const rows = useSelector((store) => store.staffs.list.rows);
  const count = useSelector((store) => store.staffs.list.count);
  const modalOpen = useSelector((store) => store.staffs.list.modalOpen);
  const idToDelete = useSelector((store) => store.staffs.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/staffs/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/staffs/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Staffs</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Staffs'
                    : 'http://localhost:8080/api-docs/#/Staffs'
            }
        >
            API documentation for staffs
        </a>
    </div>
    <br/>
