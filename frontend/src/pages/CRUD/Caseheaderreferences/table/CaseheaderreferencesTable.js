...
import * as dataFormat from 'pages/CRUD/Caseheaderreferences/table/CaseheaderreferencesDataFormatters';

import actions from 'actions/caseheaderreferences/caseheaderreferencesListActions';
...
const CaseheaderreferencesTable = () => {
  const [filters, setFilters] = React.useState([
    { label: 'Ref No', title: 'refno' },
    { label: 'Related Cases', title: 'relatedcases' },
    { label: 'Time Bar', title: 'timebar' },
    { label: 'Introducers', title: 'introducers' },
    { label: 'Year Of Creation', title: 'yearofcreation' },
    { label: 'Case No', title: 'caseno' },
    { label: 'Handlers', title: 'handlers' },
    { label: 'Supervisors', title: 'supervisors' },
    { label: 'Staffs', title: 'staffs' },
    { label: 'DLA', title: 'dla' },
    { label: 'Money Laundry Compliance', title: 'moneylaundrycompliance' },
    { label: 'Temp Ref No', title: 'temprefno' },
    { label: 'Status', title: 'status' },
    { label: 'Remarks', title: 'remarks' },
    { label: 'Follow Up Tasks', title: 'followuptasks' },
    { label: 'Create By', title: 'createby' },
    { label: 'Update By', title: 'updateby' },
    { label: 'Case Progress ID', title: 'caseprogressid', number: 'true' },
    { label: 'Client ID', title: 'clientid', number: 'true' },
    { label: 'File Location ID', title: 'casefilelocationid', number: 'true' },
    { label: 'Case Types ID', title: 'casetypesid', number: 'true' },
    { label: 'Conflict Check', title: 'conflictcheck', number: 'true' },
    { label: 'Client Check', title: 'clientcheck', number: 'true' },
    { label: 'Progress', title: 'caseprogress', number: 'true' },
    { label: 'Work Accident', title: 'workaccident', number: 'true' },
    { label: 'Liability Admitted', title: 'liabilityadmitted', number: 'true' },
    { label: 'Approval Status', title: 'approvalstatus', number: 'true' },
  ]);
  ...
  const loading = useSelector(
    (store) => store.caseheaderreferences.list.loading,
  );
  const rows = useSelector((store) => store.caseheaderreferences.list.rows);
  const count = useSelector((store) => store.caseheaderreferences.list.count);
  const modalOpen = useSelector(
    (store) => store.caseheaderreferences.list.modalOpen,
  );
  const idToDelete = useSelector(
    (store) => store.caseheaderreferences.list.idToDelete,
  );
      ...
      <div>
        {null && (
          <Button
            ...
            size='xs'
            onClick={() =>
              dispatch(push(`/admin/caseheaderreferences/${cell}`))
            }
          >
            View
          </Button>
        ...
        <Button
          ...
          size='xs'
          onClick={() =>
            history.push(`/admin/caseheaderreferences/${cell}/edit`)
          }
        >
          Edit
        </Button>
  ...
  const columns = [
    ...
    {
      dataField: 'caseprogressid',
      sort: true,

      text: 'Case Progress ID',
    },
  ...
  return (
    <div>
      <Widget title={<h4>Caseheaderreferences</h4>} collapse close>
        <div>
          <a
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Caseheaderreferences'
                : 'http://localhost:8080/api-docs/#/Caseheaderreferences'
            }
          >
            API documentation for caseheaderreferences
          </a>
        </div>
        <br />

        <Link to='/admin/caseheaderreferences/new'>
...
export default CaseheaderreferencesTable;
