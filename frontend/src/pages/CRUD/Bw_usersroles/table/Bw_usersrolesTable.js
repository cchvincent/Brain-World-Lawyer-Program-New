...
import * as dataFormat from 'pages/CRUD/Bw_usersroles/table/Bw_usersrolesDataFormatters';

import actions from 'actions/bw_usersroles/bw_usersrolesListActions';
...
const Bw_usersrolesTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Create By', title: 'createby'},{label: 'Update By', title: 'updateby'},
    {label: 'User ID', title: 'userid', number: 'true'},{label: 'Role ID', title: 'roleid', number: 'true'},
  ...
  const loading = useSelector((store) => store.bw_usersroles.list.loading);
  const rows = useSelector((store) => store.bw_usersroles.list.rows);
  const count = useSelector((store) => store.bw_usersroles.list.count);
  const modalOpen = useSelector((store) => store.bw_usersroles.list.modalOpen);
  const idToDelete = useSelector((store) => store.bw_usersroles.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/bw_usersroles/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/bw_usersroles/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Bw_usersroles</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Bw_usersroles'
                    : 'http://localhost:8080/api-docs/#/Bw_usersroles'
            }
        >
            API documentation for bw_usersroles
        </a>
    </div>
    <br/>

<Link to="/admin/bw_usersroles/new">
  <button
...
export default Bw_usersrolesTable;
