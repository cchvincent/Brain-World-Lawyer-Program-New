...
router.post('/', async (req, res) => {
    await CasetypesService.create(req.body.data, req.currentUser, true, req.headers.referer);
    const payload = true;
...
router.get('/', wrapAsync(async (req, res) => {
  const payload = await CasetypesDBApi.findAll(
    req.query,
  );
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await CasetypesDBApi.findBy(
    { id: req.params.id },
  );
